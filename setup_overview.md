# Gold Standard Setup Overview

This document provides a comprehensive overview of the Gold Standard project, its setup, and its operational components. It is intended to be the ultimate source of truth for understanding how the system works.

## 1. Project Overview

Gold Standard is a comprehensive end-to-end system for quantitative analysis of precious metals and intermarket assets. It combines real-time market data, technical indicators, economic calendar intelligence, and Google Gemini AI to generate structured trading reports.
...
-   **Service (`gold-standard-daily.service`)**:
    ...
    -   **User**: `ali_shakil_backup`
    -   **Command**: `/usr/bin/flock -n /tmp/gold_standard.lock /home/ali_shakil_backup/codex.sh run`
        -   The `flock` command ensures that only one instance of the daily analysis can run at a time.
        -   The `/home/ali_shakil_backup/codex.sh run` script is the entry point for the daily analysis.
    -   **Logging**: All output is logged to `/home/ali_shakil_backup/gold_standard_config/run.log`.
...
-   **Service (`gold-standard-weekly-cleanup.service`)**:
    ...
    -   **Logging**: All output is logged to `/home/ali_shakil_backup/gold_standard_config/cleanup.log`.
