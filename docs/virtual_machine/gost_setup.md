...
- **Description**: "Gold Standard Docker Compose Stack"
- **User**: `root`
...
- **Service (`gold-standard-daily.service`)**:
    ...
    - **User**: `user`
    - **Command**: `/usr/bin/flock -n /tmp/gold_standard.lock /home/user/codex.sh run`
    ...
    - **Logging**: All output is logged to `/home/user/gold_standard_config/run.log`.

- **Timer (`gold-standard-daily.timer`)**:
    - **Description**: "Run Gold Standard Daily Analysis at 5am"
...
- **Service (`gold-standard-weekly-cleanup.service`)**:
    - **Description**: "Gold Standard Weekly Cleanup"
    - **Dependencies**: Requires `docker.service` and `network-online.target`.
    - **Command**: `/usr/local/bin/gold-standard-weekly-cleanup.sh`
    - **Logging**: All output is logged to `/home/user/gold_standard_config/cleanup.log`.
...
To enforce this, the `docker-compose.yml` file has been configured to use **bind mounts** to a dedicated directory on a separate data disk (`/mnt/newdisk`). All persistent data generated by the Docker containers is stored in the `/mnt/newdisk/gold_standard/docker-data` directory.
...
- **`gost`**: The main Gold Standard application.
...
The `docker-compose.yml` file has been modified to use bind mounts for all persistent data. Instead of using Docker's default named volumes (which are stored on the root filesystem), each service now mounts a subdirectory within `/mnt/newdisk/gold_standard/docker-data`.
...
> `codex.sh` is a utility script designed to simplify interaction with the Gold Standard Docker Compose project. It provides a set of high-level commands to manage the project's lifecycle, including running analysis, monitoring logs, and building services.
...
| Command | Description |
| :------ | :---------- |
| `run`   | Execute the `gold_standard` project for a single analysis cycle, including post-analysis tasks and Notion publishing. This is a one-shot operation suitable for `systemd` timers. |
...
| `build` | Rebuild the Docker image(s) for the `gold_standard` project. This ensures that any changes to the application code or Dockerfile are incorporated. |
...
*   **`DOCKER_COMPOSE_FILE`**: `/mnt/newdisk/gold_standard/docker-compose.yml`
*   **`SERVICE_NAME`**: `gost` (the main Gold Standard application service)
