...
ENV_FILE="/mnt/disk/gold_standard/.env"
PROJECT="/mnt/disk/gold_standard"
...
if [ "$EXIT_CODE" -ne 0 ]; then
  ...
  /bin/systemctl restart gold-standard-run-once.service || true
  # Fire an alert via Python notifier (non-fatal if notifier fails)
  python - <<PY || true
from scripts.notifier import send_discord
send_discord('Gold Standard healthcheck failed on host; attempted restart of run-once service')
PY
  exit "$EXIT_CODE"
fi
